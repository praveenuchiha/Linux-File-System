...
def load_fs_special_files():
  ...
  try:
	...
	def Creating_superblocks():

 Metadata_SuperBlock['Superblock'] = {'creationtime' : 137895465, 'mounted': 50, 'dev_id': 20, 
                       'freestart':1, 'freeend':25, 'root': 26, 'maxblocks':10000}
  persist_metadata(FileName_SuperBlock)


def Creating_freeblockList():
   Counter_freeBlockList = 1
   i = 1;
   for count in range (27:10000):
      if(count%400 = 0):
        for i in range(count):
           freeBlockList[block_Number] = "T" 
        i = count + 1
        persist_metadata('linddata.'+str(Counter_freeBloclList), DataTable_freeBlockList)
        freeBloclListCounter++


def Creating_datablocks():
   for count in range (27:9999):
        persist_metadata('linddata.'+str(Counter_freeBloclList), null)



def findEmptyBlock(i):
  data = openfile('linddata.'+str(i),True)
  datastring = metadatafo.readat(None, 0)
  data.close()

  # get the dict we want
  metadata_desired = deserializedata(datastring)

  for key in metadata_desired:
    if(metadata_desired[key]=='T')
       return key
    else
       findEmptyBlock(i+1)
...
def _blank_fs_init():
  ...
  Creating_superblocks()
  Creating_freeblockList()
  Creating_datablocks()
...
def rmdir_syscall(path):
  ...
  try:
	...
	initialize = 1
    blockNumber = findEmptyBlock(initialize)
    opentheblock = open('linddata.'str(thisinode), true)
    opentheblock.writeat(directorynode)
    opentheblock.close()   

    obj = open('linddata.'str(thisinode), true)
    linddata+str(blockNumber)['freeBlockList']['blockNumber'] = 'false'
    obj.close()
...
def open_syscall(path, flags, mode):
  ...
  try:
	...
	filedatanode = {}
      if(size > 4096)
           filedatanode = {'size':size, 'uid':DEFAULT_UID, 'gid':DEFAULT_GID, 'mode':33261, 
          'linkcount':2, 'atime':1323630836, 'ctime':1323630836, 'mtime':1323630836, ‘indirect’:2 ‘location’:2444}

      if(size<4096)
          filedatanode = {'size':size, 'uid':DEFAULT_UID, 'gid':DEFAULT_GID, 'mode':33261, 
          'linkcount':2, 'atime':1323630836, 'ctime':1323630836, 'mtime':1323630836, ‘indirect’:1 ‘location’:2444}

       initialize = 1
       blockNumber = findEmptyBlock(initialize)
       opentheblock = open('linddata.'str(blockNumber), true)
       opentheblock.writeat(filedatanode)
       opentheblock.close()  
